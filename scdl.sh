...
if $curlinstalled; then
  echo "Using" `curl -V` | cut -c-21
elif $wgetinstalled; then
  echo "Using" `wget -V` | cut -c-24
  echo "cURL is preferred" 
else
...
if [[ "$(echo "$url" | cut -d "/" -f 4)" == "" ]] ; then
    ...
    echo " Detected download type : All of one user's songs"
    downallsongs 
elif [[ "$(echo "$url" | cut -d "/" -f 5)" == "sets" ]] && [[ "$(echo "$url" | cut -d "/" -f 6)" == "" ]] ; then
    echo " Detected download type : All of one user's sets"
    downallsets 
elif [[ "$(echo "$url" | cut -d "/" -f 5)" == "sets" ]] ; then
    echo " Detected download type : One single set"
    downset
else
    echo " Detected download type : One single song"
    downsong
fi
